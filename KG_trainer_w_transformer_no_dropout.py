...
def batch_rgcn(rgcn_layer, node_features_batch, edge_index_batch, edge_type_batch):
    ...
    x = node_features_batch.view(B * N, C)
    edge_index_list = []
    edge_type_list = []
    for i in range(B):
    ...
    edge_type_combined = torch.cat(edge_type_list, dim=0)
...
class LearnedSAGPooling(nn.Module):
    ...
    def forward(self, node_features, edge_index, edge_type):
        ...
        scores = self.scoring(node_features, edge_index, edge_type)
        k = max(1, int(N * self.compress_ratio))
        _, topk_indices = torch.topk(scores, k, dim=1)
        topk_indices_expanded = topk_indices.unsqueeze(-1).expand(-1, -1, C)
        x_top = torch.gather(node_features, 1, topk_indices_expanded)
        attn_scores = self.attn(x_top).squeeze(-1)
        attn_weights = F.softmax(attn_scores, dim=1).unsqueeze(-1)
        pooled = torch.sum(x_top * attn_weights, dim=1, keepdim=True)
        return pooled, x_top, topk_indices

# -----------------------------------------------------------
# Graph Encoder with Learned Compression (Batched RGCN-based)
# -----------------------------------------------------------
class GraphEncoder(nn.Module):
    def __init__(self, hidden_size, num_concepts, compress_ratio=0.5):
        ...
        self.gcn = GraphConvolutionalLayer(hidden_size, hidden_size, num_relations)
    ...
    def forward(self, concept_ids, edge_index, edge_type):
        x = self.embedding(concept_ids)
        updated_nodes = batch_rgcn(self.gcn, x, edge_index, edge_type)
        pooled, _, topk_indices = self.sag_pool(updated_nodes, edge_index, edge_type)
        concept_ids_exp = concept_ids.unsqueeze(-1)
...
class BartGraphAwareForConditionalGeneration(BartForConditionalGeneration):
    def __init__(self, config: BartConfig, tokenizer: BartTokenizer):
        ...
        self.tokenizer = tokenizer  # Used to convert concept ids to text.
        self.post_init()
...
class GraphAwareDataCollator(DataCollatorForSeq2Seq):
    def __call__(self, features):
        ...
        if "concept_ids" in graph_features:
            concept_ids_list = [f if isinstance(f, torch.Tensor) else torch.tensor(f, dtype=torch.long)
                                for f in graph_features["concept_ids"]]
            batch["concept_ids"] = nn.utils.rnn.pad_sequence(concept_ids_list, batch_first=True, padding_value=0)
        if "edge_index" in graph_features:
            edge_index_list = [f if isinstance(f, torch.Tensor) else torch.tensor(f, dtype=torch.long)
        ...
        if "edge_type" in graph_features:
            edge_type_list = [f if isinstance(f, torch.Tensor) else torch.tensor(f, dtype=torch.long)
