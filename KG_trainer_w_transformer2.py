...
def batch_rgcn(rgcn_layer, node_features_batch, edge_index_batch, edge_type_batch):
    ...
    # Reshape (B, N, C) -> (B*N, C)
    # If needed, ensure contiguous before view
    ...
    edge_index_combined = torch.cat(edge_index_list, dim=1)  # (2, sum over B of E)
    edge_type_combined = torch.cat(edge_type_list, dim=0)    # (sum over B of E)
...
# -----------------------------------------------------------
# Graph Encoder with Global Transformer
# -----------------------------------------------------------
class GraphEncoder(nn.Module):
    def __init__(self, hidden_size, num_concepts, compress_ratio=0.5, num_global_layers=1, nhead=8):
        ...
        # global transformer to allow each node to attend to all others
        enc_layer = nn.TransformerEncoderLayer(d_model=hidden_size, nhead=nhead)
    ...
    def forward(self, concept_ids, edge_index, edge_type):
        ...
        # global transformer: expects shape (N, B, E)
